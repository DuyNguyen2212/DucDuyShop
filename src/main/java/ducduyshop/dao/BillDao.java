...
import ducduyshop.mapper.BillMapper;
...
public class BillDao extends BaseDao {
	...
	public List<Bill> getPurchasedBill() {
		String sql = "SELECT * FROM `bill` WHERE bill.status = 1";	
		List<Bill> list = jdbcTemplate.query(sql, new BillMapper());
		
		return list;
	}
	
	public List<Bill> getUndoneBill() {
		String sql = "SELECT * FROM `bill` WHERE bill.status = 0";	
		List<Bill> list = jdbcTemplate.query(sql, new BillMapper());
