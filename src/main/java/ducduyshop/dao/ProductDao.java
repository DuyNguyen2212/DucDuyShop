...
import ducduyshop.mapper.ProductDtoMapper;
import ducduyshop.mapper.ProductMapper;
...
//	Product
	...
	public StringBuffer createBuffer(Product product) {
		...
		sql.append("(`id`, `category_id`, `company_id`, `name`, `image`, `brief`, `material`, `quantity`, `price`, `description`, `sale_off`, `is_new`, `is_topsell`, `createat`) ");
		sql.append("VALUES ");
		sql.append("(NULL, ");
	...
	public StringBuffer editBuffer(Product product) {
		...
		sql.append("`brief`='" + product.getBrief() + "', ");
	...
	public List<Product> getProductsByPriceForPagination(int from, int to, int start, int limit) {
		String sql = "SELECT * FROM `product` WHERE price BETWEEN " + from + " AND " + to + " LIMIT " + start + ", " + limit;
		List<Product> list = jdbcTemplate.query(sql, new ProductMapper());
		return list;
	}
}
